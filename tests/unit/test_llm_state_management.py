...
def test_llm_services_bind_mount_models_and_configure_slot_state_path():
    ...
    for service_name in ("llm", "llm-cuda"):
        ...
        assert "MODEL_STATE_DIR_NAME=\"$$(printf '%s' \"$$MODEL_FILE_NAME\" | tr '.' '-')\";" in command
...
def test_llm_services_use_posix_safe_state_dir_derivation():
    repo_root = Path(__file__).resolve().parents[2]
    compose_text = (repo_root / 'docker-compose.yml').read_text(encoding='utf-8')

    assert '${MODEL_FILE_NAME//./-}' not in compose_text
    assert "tr '.' '-'" in compose_text
