...
def _persist_finished_result(db: Session, *, job_id: str, session_id: str | None, user_message_id: str | None, result: dict, now: datetime) -> None:
...
def get_job(job_id: str, db: Session = Depends(get_db)):
    ...
    if status == "finished":
        result = (j.result if j else None) or (rec.result_json if rec else None) or {}
        resp["result"] = result
        if rec:
            rec.result_json = result
            rec.error_json = {}
        _persist_finished_result(
            db,
            job_id=job_id,
            session_id=session_id,
            user_message_id=user_message_id,
